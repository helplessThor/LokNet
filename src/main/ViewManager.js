...
export class ViewManager {
    constructor(window) {
        ...
        this.nextId = 1;
        this.bounds = { x: 0, y: 90, width: 800, height: 600 };
        this.pendingPermissions = new Map();
        this.transientPermissions = new Map();
        this.window = window;
    ...
    setupPermissionHandler() {
        ...
        session.defaultSession.setPermissionCheckHandler((webContents, permission, origin, details) => {
            ...
            try {
                if (!origin)
                    return true; // Default allow for null origin? Or handle gracefully
                ...
                if (permission === 'media') {
                    ...
                    if (types.includes('video'))
                        permsToCheck.push('camera');
                    if (types.includes('audio'))
                        permsToCheck.push('microphone');
        ...
        session.defaultSession.setPermissionRequestHandler((webContents, permission, callback, details) => {
            ...
            try {
                ...
                if (permission === 'media') {
                    ...
                    permissionsToHandle = [];
                    if (types.includes('video'))
                        permissionsToHandle.push('camera');
                    if (types.includes('audio'))
                        permissionsToHandle.push('microphone');
